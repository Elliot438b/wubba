...
ACTION gamemallards::delsymbol(name code, string sym, asset minperbet)
{
    ...
    extended_symbol cur_ex_sym = extended_symbol(symB, code);
    auto itr = tableround.begin();
    while (itr != tableround.end())
    {
        eosio::check(itr->amountSymbol != cur_ex_sym, "Symbol still be used!");
        itr++;
    }
...
ACTION gamemallards::serverseed(uint64_t tableId, checksum256 encodeSeed)
{
    ...
    {
        // fluency support
        eosio::check(existing->tableStatus == (uint64_t)table_stats::status_fields::ROUND_END || existing->tableStatus == (uint64_t)table_stats::status_fields::ROUND_START, "The currenct round isn't ROUND_END or ROUND_START!");
        if (existing->dealerBalance < existing->oneRoundDealerMaxPay * 2)
        {
            INLINE_ACTION_SENDER(gamemallards, pausetablesee)
            (
...
ACTION gamemallards::closetable(uint64_t tableId)
{
    ...
    auto existing = tableround.find(tableId);
    eosio::check(existing != tableround.end(), notableerr);
    eosio::check(existing->tableStatus == (uint64_t)table_stats::status_fields::ROUND_END || existing->tableStatus == (uint64_t)table_stats::status_fields::PAUSED || existing->tableStatus == (uint64_t)table_stats::status_fields::CLOSED, "The round isn't end, pause or closed, can't close!");
